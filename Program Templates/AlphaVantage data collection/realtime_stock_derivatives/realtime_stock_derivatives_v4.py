...
# Variables to Modify
PAST_DATA_POINTS = 100    #How many minutes we look back for averages
BASE_TRANSACTION = 2000.0     #How much money to invest when buying or selling stock
symbols = ['BE']   #Symbols to analyze
...
# Initialize last_price and then to hold last cycles data for comparison
j = 0 #index for highest price list (Go through then repeat, then have for loop go through and find the highest price)

now  = datetime.datetime.now()

prices = [[0 for x in range(0,PAST_DATA_POINTS)] for symbol in symbols] #Each symbol can store PAST_DATA_POINTS previous prices
...
while True:
    ...
    for symbol in symbols:
        ...
        for x in range(0, PAST_DATA_POINTS):
            if prices[i][x] > highest:
                highest = prices[i][x]
        ...
        if derivative != 0:
            print(symbol.rjust(5), '|', str(round(relative_derivative, 4)).rjust(10) ,'% |', str(round(price, 2)).rjust(8),
                  '|', str(number_shares[i]).rjust(6), "shares")
        # Update total_money based on shares bought and value
        #total_money += price * number_shares[i]

        #Store price and time temporarily
        last_price[i] = price
        then[i] = now
